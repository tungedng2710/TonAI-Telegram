...
import warnings
from configs import *
from utils import complete, encode_image_to_base64, check_ollama_model, gen_image
...
def trigger_image_generator(message):
    ...
    bot.reply_to(message, "What do you want to generate?")
...
def handle_active_bot(message):
    ...
    if user_session['active']:
        ...
        if message.chat.type == 'private':
            ...
            if USER_SESSIONS[message.chat.id]["features"]["gen_image"]:
                gen_image(prompt=input_text, chat_id=chat_id)
                saved_path = f"stuffs/{chat_id}_temp.jpg"
                with open(saved_path, 'rb') as photo:
                    bot.send_photo(chat_id, photo, caption="Here is your image!")
                user_session["dialogue"].append({"role": "assistant", 
                                                 "content": "Image generated by Stable Diffusion 3.5",
                                                 "images": [encode_image_to_base64(saved_path)]})
                USER_SESSIONS[message.chat.id]["features"]["gen_image"] = False
